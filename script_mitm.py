...
def response(flow):
	...
	if flow.request.query.get('mime') != None:
		...
		data = json.dumps({'Header':'VideoPlayBack','timestamp':st,'url':flow.request.headers.get('referer'),'ip':str(ip),'player':flow.request.query.get('itag'), 
		'type':typeObject, 'numPaquet':flow.request.query.get('rn'), 'bufferDispo':flow.request.query.get('rbuf'), 'taillePaquet':round(flength,2)})
		array.append(data)
	elif flow.request.query.get('state') != None:
		data = json.dumps({'Header':'Stats','timestamp':st,'url':flow.request.headers.get('referer'),'ip':str(ip),'player':flow.request.query.get('cplayer'), 
		'state':flow.request.query.get('state'), 'navigateur':flow.request.query.get('cbr')})
		array.append(data)
	else:
		typeObject = None
	print('array: ',array)
