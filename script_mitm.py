...
def response(flow):
    ...
    if 'youtube.com' in url:
        ...
        dict = {
                ...
                'ip':           str(ip),                                    # ip server
                'url':          flow.request.url,                           # itag contains infos player
                'contentType':  flow.response.headers.get('Content-Type'),   # type of flow (audio, video, ...)
                'contentLength':round(flength,2),                          # content length 
                } 
        http_url = flow.request.url
        liste.append(dict)
        print(dict)
    ...
    if flow.request.query.get('mime') != None:                           
        ...
        dict = {
                ...
                'taillePaquet': round(flength,2),                           # content length
